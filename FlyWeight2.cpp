...
{
public:
    Flyweight(const shared_ptr<SharedData> sharedData) : mp_sharedData(sharedData) {}
...
private:
    shared_ptr<SharedData> mp_sharedData;
};
...
public:
    FlyweightFactory(initializer_list<SharedData> shareDatas) {
        ...
        {
            m_flyweights.insert(make_pair<string, Flyweight>(GetKey(element), Flyweight(make_shared<SharedData>(element))));
        }
    }
    Flyweight GetFlyweight(const SharedData& sharedData) {
        ...
        if (m_flyweights.find(key) == m_flyweights.end()) {
            ...
            m_flyweights.insert(make_pair(key, Flyweight(make_shared<SharedData>(sharedData))));
        } else {
...
int main()
{
    unique_ptr<FlyweightFactory> factory(new FlyweightFactory({{"Chevrolet", "Camaro2018", "pink"}, {"Mercedes Benz", "C300", "black"}, {"Mercedes Benz", "C500", "red"}, {"BMW", "M5", "red"}, {"BMW", "X6", "white"}}));

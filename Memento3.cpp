...
public:
    ...
    unique_ptr<Memento> save() {
        return unique_ptr<ConcreteMemento>(new ConcreteMemento(m_state));
    }
...
public:
    ...
    void undo() {
        ...
        // Memento* memento = m_mementos.back().get();
        unique_ptr<Memento> memento = move(m_mementos.back());
        m_mementos.pop_back();
...
private:
    vector<unique_ptr<Memento>> m_mementos;
